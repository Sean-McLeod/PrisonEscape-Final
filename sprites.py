...
class Sprites:
    ...
    def sprite_animation(self, sprite_list):
        ...
        if self._counter > 5:
            ...
            if self._current_sprite >= len(sprite_list):
                self._current_sprite = 0
        self._screen.blit(sprite_list[self._current_sprite], (constants.DRAGON_X, constants.DRAGON_Y))
        self.set_rect(pygame.Rect(
            constants.DRAGON_X, constants.DRAGON_Y, self._width, self._height
        ))

    def portal_animation(self, sprite_list):
        self._counter += 1
        if self._counter > 2:
            self._counter = 0
            self._current_sprite += 1
            if self._current_sprite >= len(sprite_list):
                self._current_sprite = 8
        self._screen.blit(sprite_list[self._current_sprite], (constants.SECOND_DOOR_X, constants.SECOND_DOOR_Y))
        # self.set_rect(pygame.Rect(
        #     constants.DRAGON_X, constants.DRAGON_Y, self._width, self._height
        # ))
    ...
    def restrict_motion(self, x_motion, y_motion):
        if (
            self._rect.x >= constants.SCREEN_WIDTH - self._sprite.get_width() - x_motion
            or self._rect.x <= 0 + x_motion
            or self._rect.y >= constants.SCREEN_HEIGHT - self._sprite.get_height() - y_motion
            or self._rect.y <= 0 + y_motion
        ):
            # self._rect.clamp_ip(self._screen.get_rect())
            return True
        else:
